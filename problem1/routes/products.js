...
import { prodcreateqr, getproducts, getprodbyname, getallprod, deleteprod } from "../querryies/dbquerryes.js"
...
router.post('/', async (req, res) => {
    try {
        ...
        connection.query(getproducts, [plu], (err, results) => {
            ...
            if (results.length != 0 && results[0].product_plu == plu) {
                return res.status(200).json(`Продукт с таким артикулем уже есть под ID: ${results[0].id}`);
            }
            const uid = uuidv4()
